...
public class AggregationTests {
	...
	public void letShouldBeAppliedCorrectly() {
		...
		ExpressionVariable discounted = ExpressionVariable.newVariable("discounted").forExpression(ConditionalOperator.newBuilder().when(Fields.field("applyDiscount")).then(0.9D).otherwise(1.0D));

		TypedAggregation<Sales2> agg = Aggregation.newAggregation(Sales2.class,
				Aggregation.project()
						.and(Let.define(total, discounted).andApply(AggregationFunctionExpressions.MULTIPLY.of(Fields.field("total"), Fields.field("discounted"))))
						.as("finalTotal"));
