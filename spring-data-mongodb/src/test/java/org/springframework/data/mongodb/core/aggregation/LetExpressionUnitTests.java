...
import static org.springframework.data.mongodb.core.aggregation.AggregationExpressions.Let.*;
...
public class LetExpressionUnitTests {
	...
	public void shouldConstructLetExpressionCorrectly() {
		...
		TypedAggregation<Sales> agg = Aggregation.newAggregation(Sales.class,
				Aggregation.project()
						.and(define(newVariable("total").forExpression(ADD.of(Fields.field("price"), Fields.field("tax"))),
								newVariable("discounted").forExpression(
										ConditionalOperator.newBuilder().when(Fields.field("applyDiscount")).then(0.9D).otherwise(1.0D)))
												.andApply(MULTIPLY.of(Fields.field("total"), Fields.field("discounted")))) //
						.as("finalTotal"));
	...
	public void shouldConstructLetExpressionCorrectlyWhenUsingLetOnProjectionBuilder() {
		...
		ExpressionVariable var1 = newVariable("total").forExpression(ADD.of(Fields.field("price"), Fields.field("tax")));
		...
		AggregationExpression in = MULTIPLY.of(Fields.field("total"), Fields.field("discounted"));
